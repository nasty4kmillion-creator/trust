<?php
class ftt_WP_Widget_Videos_Block extends WP_Widget {
	/**
	...
	public function __construct() {
		$widget_options = array(
			'classname'   => 'widget-videos-block',
			'description' => __( 'Display blocks of videos sorted by views, date, popularity, category, etc.', 'wpst' ),
		);
		parent::__construct( 'widget-videos-block', 'FamousTube - Video Blocks', $widget_options );
	}
	public function widget( $args, $instance ) {
		...
		$w  = isset( $instance['widget_id'] ) ? $instance['widget_id'] : '';
		...
		if ( $home_query->have_posts() ) : ?>
			...
			if ( $tv == 'related' ) {
					global $post;
					// $post_cat = wp_get_post_categories( $post->ID ); // var_dump($post_cat);
					$category = get_the_category();
					$cat_link = get_category_link( $category[0]->term_id );
			}
			?>
			<?php if ( $tv != 'related' ) : ?>
	<a class="more-videos label" href="<?php echo get_bloginfo( 'url' ); ?>/?filter=<?php echo $tv; ?>
													<?php
													if ( $cv != 0 ) :
														?>
		&amp;cat=<?php echo $cv; ?><?php endif; ?>"><i class="fa fa-plus"></i> <span><?php _e( 'More videos', 'wpst' ); ?></span></a>
	<?php endif; ?>

	<div class="video-loop">
	<div class="row no-gutters">
		<div class="order-1 order-sm-1 order-md-1 order-lg-1 order-xl-1 col-12 col-md-6 col-lg-6 col-xl-6">
			<?php if ( '' !== $advertising && wp_count_posts() > '1' ) : ?>
		...
		<?php endif; ?>
			...
			<?php
			$video_counter = 0;
			set_query_var( 'video_loop_has_ad', ( '' !== $advertising ) );
			while ( $home_query->have_posts() ) :
				++$video_counter;
				set_query_var( 'video_counter', $video_counter );
				$home_query->the_post();
				get_template_part( 'loop-templates/loop', 'video-widget' );
			endwhile;
			?>
	</div>
			<?php if ( $tv == 'related' ) : ?>
		<a class="more-videos label" href="<?php echo $cat_link; ?>"><span><?php _e( 'Load more related videos', 'wpst' ); ?></span></a>
	<?php endif; ?>
	</div>
...
function ftt_register_widgets() {
	register_widget( 'ftt_WP_Widget_Videos_Block' );
}
	add_action( 'widgets_init', 'ftt_register_widgets' );
