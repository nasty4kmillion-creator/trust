...
function ftt_post_like() {
	...
	if ( isset( $_POST['post_like'] ) ) {
		...
		if ( ! ftt_hasAlreadyVoted( $post_id ) ) {
			...
			update_post_meta( $post_id, 'rate', intval( ftt_getPostLikeRate( $post_id ) ) );
...
add_action( 'wp_ajax_nopriv_post-like', 'ftt_post_like' );
add_action( 'wp_ajax_post-like', 'ftt_post_like' );
