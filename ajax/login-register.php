<?php
function ftt_login_register_modal() {
					...
					<a href="#" class="close" data-dismiss="modal" aria-label="Close"><i class="fa fa-remove"></i></a>
						...
						<div class="wpst-register">	
							<?php if ( get_option( 'users_can_register' ) ) : ?>						 
								...
								<form id="ftt_registration_form" action="<?php echo home_url( '/' ); ?>" method="POST">
									...
									<div class="form-field">
										<label for="ftt_user_email"><?php esc_html_e( 'Email', 'wpst' ); ?></label>
										<input class="form-control input-lg required" name="ftt_user_email" id="ftt_user_email" type="email"/>
									</div>
									<div class="form-field">
										<label for="ftt_user_pass"><?php esc_html_e( 'Password', 'wpst' ); ?></label>
						...
						<div class="wpst-login">							 
							...
							<form id="ftt_login_form" action="<?php echo home_url( '/' ); ?>" method="post">
						...
						<div class="wpst-reset-password">							 
							...
							<form id="ftt_reset_password_form" action="<?php echo home_url( '/' ); ?>" method="post">
								<div class="form-field">
									<label for="ftt_user_or_email"><?php esc_html_e( 'Username or E-mail', 'wpst' ); ?></label>
...
add_action( 'wp_footer', 'ftt_login_register_modal' );
...
// LOGIN
function ftt_login_member() {

	// Get variables
	$user_login = $_POST['ftt_user_login'];
	$user_pass  = $_POST['ftt_user_pass'];
...
add_action( 'wp_ajax_nopriv_ftt_login_member', 'ftt_login_member' );



// REGISTER
function ftt_register_member() {
	$siteKey = xbox_get_field_value( 'wpst-options', 'recaptcha-site-key' );
	$secret  = xbox_get_field_value( 'wpst-options', 'recaptcha-secret-key' );

	// Get variables
	$user_login = $_POST['ftt_user_login'];
	$user_email = $_POST['ftt_user_email'];
	$user_pass  = $_POST['ftt_user_pass'];
...
add_action( 'wp_ajax_nopriv_ftt_register_member', 'ftt_register_member' );


// RESET PASSWORD
function ftt_reset_password() {

		// Get variables
		$username_or_email = $_POST['ftt_user_or_email'];
	...
	elseif ( empty( $username_or_email ) ) {
		...
		$user_forgotten = ftt_lostPassword_retrieve( $username );
...
add_action( 'wp_ajax_nopriv_ftt_reset_password', 'ftt_reset_password' );


function ftt_lostPassword_retrieve( $user_data ) {
 ...
 */
/*
add_filter( 'wp_nav_menu_items', 'ftt_login_link_to_menu', 10, 2 );
function ftt_login_link_to_menu ( $items, $args ) {
